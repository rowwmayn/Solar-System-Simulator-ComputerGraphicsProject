...
def draw_planet(planet_idx, radius, rotation_angle, axial_tilt):
    ...
    # # Surface features
    # if planet_idx == 1:  # Mercury craters
    #     for (lat, lon), color in [((20,10),(0.5,0.5,0.5)), ((-25,130),(0.4,0.4,0.4))]:
    #         glPushMatrix()
    #         glRotatef(lon, 0, 0, 1)
    #         glRotatef(lat, 0, 1, 0)
    #         glTranslatef(radius, 0, 0)
    #         glColor3fv(color)
    #         gluSphere(gluNewQuadric(), radius*0.1, 12, 12)
    #         glPopMatrix()
...
def update_simulation():
    global comet_orbit_angle, collision_happened, collision_time
    ...
    # Check collision against all planets
      for i in range(1, len(planet_data)):
        
        planet_orbit_radius = planet_data[i][1]
        planet_radius = planet_data[i][0]
        planet_x = planet_orbit_radius * math.cos(math.radians(orbit_angles[i]))
        planet_y = planet_orbit_radius * math.sin(math.radians(orbit_angles[i]))
        
        dx = rogue_asteroid_position[0] - planet_x
        dy = rogue_asteroid_position[1] - planet_y
        
        distance = math.sqrt(dx*dx + dy*dy)
        if distance < planet_radius + rogue_asteroid_size:
            collision_happened = True
            collision_target = i  # Update collision target to the collided planet
